...
library(ltm) # For calculating Cronbach’s Alpha
library(stats)
...
Plot_alternative <- plot(likert_Qs) + ggtitle("Encountering agreement and disagreement via various media")
...
## Affective polarization and the two FBIs? [UPDATE!]
...
DF_W1$IN_PARTY <- factor(DF_W1$IN_PARTY, levels = c("Suomen Sosialidemokraattinen Puolue (SDP)", "Kansallinen Kokoomus (Kok.)", "Perussuomalaiset (PS)", "Suomen Keskusta (Kesk.)", "Vihreä liitto (Vihr.)", "Vasemmistoliitto (Vas.)", "Suomen ruotsalainen kansanpuolue (RKP)", "Suomen Kristillisdemokraatit (KD)", "Liike Nyt (Liik.)", "Muu, mikä?"), labels = c("SDP", "KOK", "PS", "KESK", "VIHR", "VAS", "RKP", "KD", "NYT", "MUU"))
...
TEMP$type <- TEMP$FBI
TEMP$type[TEMP$FBI == 0] <- "'Balanced'"
TEMP$type[TEMP$FBI < 0] <- "More disagreement"
TEMP$type[TEMP$FBI > 0] <- "More ageement"

me <- mean(TEMP$API, na.rm = T)

ggplot(TEMP, aes(FBI, API))+
  geom_point(aes(colour = TEMP$type)) +
  geom_smooth(method=lm) +
  geom_hline(yintercept= me, linetype="dashed", alpha = 0.6) +
  scale_color_manual(values=c("#1f60ab", "#47ab1f", "#ab1f1f")) +
  theme_bw()
...
Arto also suggested that to see what happens if we remove the two most extreme cases on the top left corner, where the two data points have very high API score and a high negative FBI score:

```{r echo=FALSE, message=FALSE, warning=FALSE}

TEMP <- subset(TEMP, TEMP$FBI > -5)
...
Sure enough, that has a significant impact and reduces Pearsons correlation by 56 % and Kendall Tau by 58 %. Unsurprisingly neither of them are statistically significant anymore.
...
TEMP$type <- TEMP$FBI
TEMP$type[TEMP$FBI == 0 | TEMP$FBI == -1 | TEMP$FBI == 1] <- "'Balanced'"
TEMP$type[TEMP$FBI < -1] <- "More disagreement"
TEMP$type[TEMP$FBI > 1] <- "More ageement"
...
> When I do have discussions about politics or other societal issues with a person who I think does share the same basic values as I do, I often find that these discussions tend to turn into…

**Discussions with a person who DOES NOT SHARE the same values:**

> When I do have discussions about politics or other societal issues with a person who I think did not share the same basic values as I do, I often find that these discussions tend to turn into…
...
Lets also calculate Cronbach’s alpha for the two constructs to see how internally consistent the items are:
...
Ok, here are a couple of new things that I have added here based on Arto's suggestions, which were:

>Arguably, the link between (informational) filter bubbles and IP could be stronger than them and AP, so I'd suggest checking if there is a relationship.

- Plot FBI against the policy preference questions
- forming two questions: a GAL-TAN index based on a subset of them, and a socioeconomic index and seeing if FBI has an effect on them
- doing the same, except using the self-reported left-right and liberal-conservative scales

I will do these in two parts. First, I will check how the DEFAULT survey FBI interacts with the policy questions we had in the Wave 1 pilot survey. Second, I will do the whole prosedure again for the ALTERNATIVE survey so that we can compare them and see if there are meanigfull differences.

As a reminder, the policy questions were:

>**Seuraavassa on lueteltu eräitä ehdotuksia koskien Suomen tulevaisuuden suuntaa. Mitä mieltä olet näistä ehdotuksista?** Käytä vastaamisessa asteikkoa nollasta kymmeneen (0-10), jossa nolla tarkoittaa "erittäin huono ehdotus", viisi "ei hyvä eikä huono ehdotus" ja kymmenen "erittäin hyvä ehdotus". 

1. [GAL-TAN] Monikulttuurinen Suomi, jossa suhtaudutaan suvaitsevaisesti muista maista tuleviin ihmisiin
2. [GAL-TAN] Suomi, jossa kristilliset arvot ovat suuremmassa roolissa
3. [GAL-TAN] Suomi, jossa miehet ja naiset ovat keskenään tasa-arvoisempia
4. [GAL-TAN] Suomi, jossa vahvistetaan seksuaalisten vähemmistöjen oikeuksia
5. [GAL-TAN] Suomi, jossa on enemmän lakia ja järjestystä
6. [GAL-TAN] Suomi, jossa vahvistetaan perinteisen ydinperheen asemaa
7. [GAL-TAN] Suomi, jossa maahanmuutto on laajamittaisempaa
8. [Left-Right] Ympäristöystävällisempi Suomi, vaikka se tarkoittaisi alhaista talouskasvua tai ei kasvua ollenkaan
9. [Left-Right] Euroopan unioniin vähemmän sitoutunut Suomi
10. [Left-Right] Suomi, jossa on enemmän yksityisyrittäjyyttä ja markkinataloutta
11. [Left-Right] Suomi, jossa on alhaisempi verotus
12. [Left-Right] Suomi, jossa on pienemmät tuloerot
13. [Left-Right] Suomi, jossa on pienempi julkinen sektori
...
DF_W1 <- DF_W1[,c("ResponseId", "GAL_TAN_and_L_R_1", "GAL_TAN_and_L_R_2", "GAL_TAN_and_L_R_3", "GAL_TAN_and_L_R_4", "GAL_TAN_and_L_R_5", "GAL_TAN_and_L_R_6", "GAL_TAN_and_L_R_7", "GAL_TAN_and_L_R_8", "GAL_TAN_and_L_R_9", "GAL_TAN_and_L_R_10", "GAL_TAN_and_L_R_11", "GAL_TAN_and_L_R_12", "GAL_TAN_and_L_R_13")]
...
lables <- c("ID", var_label(DF1["ORIE_Q1"]), var_label(DF1["ORIE_Q2"]), var_label(DF1["II_Q1"]), var_label(DF1["II_Q2"]), "Filter Bubble Index", "FBI type", "Monikulttuurinen Suomi, jossa suhtaudutaan suvaitsevaisesti muista maista tuleviin ihmisiin", "Suomi, jossa kristilliset arvot ovat suuremmassa roolissa", "Suomi, jossa miehet ja naiset ovat keskenään tasa-arvoisempia", "Suomi, jossa vahvistetaan seksuaalisten vähemmistöjen oikeuksia", "Suomi, jossa on enemmän lakia ja järjestystä", "Suomi, jossa vahvistetaan perinteisen ydinperheen asemaa", "Suomi, jossa maahanmuutto on laajamittaisempaa", "Ympäristöystävällisempi Suomi, vaikka se tarkoittaisi alhaista talouskasvua tai ei kasvua ollenkaan", "Euroopan unioniin vähemmän sitoutunut Suomi", "Suomi, jossa on enemmän yksityisyrittäjyyttä ja markkinataloutta", "Suomi, jossa on alhaisempi verotus", "Suomi, jossa on pienemmät tuloerot", "Suomi, jossa on pienempi julkinen sektori")

lables <- as.character(lables)

#Setting variable labels
for(i in 1:ncol(TEMP)){
  var_label(TEMP[i]) <-  lables[i]
}
...
#### Ploting FBI against policy preference questions

Here are each of the 13 policy digestions plotted with the DEAFAULT survey FBI and with a Pearsons correlation test.

The grey dash-line is the average response to the policy suggestion, which ranged from 0 (a bad idea) to 10 (an excellent idea).

The plots are pretty consistent in that those respondents who favor GAL and LEFT policies tend to be the ones who also are more likely to encounter agreement in their day-to-day lives.

In contrast, those who favor TAN and and RIGHT policies tend to be those who report encountering more disagreement than agreement, but the dispersion also seems to be bigger. Those red dots do seem to be more widely distributed on the policy axis, where as the green ones are more tihgtly clustered.

Our "balanced" group seem to be pretty widely ditributed too, with many values on the policy axis.
...
for (i in 8:ncol(TEMP)) {
  
  title <- str_wrap(var_label(TEMP[[i]]), 60)
  me <- mean(TEMP[[i]], na.rm = T)
  cor <- cor.test(TEMP[[i]], TEMP$FBI, method = "pearson")
  foot <- paste("Pearsons correlation = ", round(cor$estimate, 2), "; p-value = ", round(cor$p.value, 4), sep = "")
  p_list_D <- append(p_list_D, cor$p.value)
  
  p <- ggplot(TEMP, aes(FBI, TEMP[[i]])) +
    geom_point(aes(colour = TEMP$type)) +
    geom_smooth(method=lm) +
    geom_hline(yintercept= me, linetype="dashed", alpha = 0.6) +
    ggtitle(title) +
...
#### Impact of FBI to GAL-TAN and LEFT-RIGHT

Ok, lets now put those policy question together in order to take a overall look at the trends. 

First, I will check how FBI is associated with the GAL-TAN dimension. Here, I used the first seven questions, and reversed the integer values so that being close to 0 always indicates GAL and being close to 10 indicates TAN.

1. [GAL-TAN, REVERSED] Monikulttuurinen Suomi, jossa suhtaudutaan suvaitsevaisesti muista maista tuleviin ihmisiin
2. [GAL-TAN] Suomi, jossa kristilliset arvot ovat suuremmassa roolissa
3. [GAL-TAN, REVERSED] Suomi, jossa miehet ja naiset ovat keskenään tasa-arvoisempia
4. [GAL-TAN, REVERSED] Suomi, jossa vahvistetaan seksuaalisten vähemmistöjen oikeuksia
5. [GAL-TAN] Suomi, jossa on enemmän lakia ja järjestystä
6. [GAL-TAN] Suomi, jossa vahvistetaan perinteisen ydinperheen asemaa
7. [GAL-TAN, REVERSED] Suomi, jossa maahanmuutto on laajamittaisempaa

```{r echo=FALSE, message=FALSE, warning=FALSE}

TEMP$GAL_TAN_and_L_R_1 <- 10 - TEMP$GAL_TAN_and_L_R_1
TEMP$GAL_TAN_and_L_R_3 <- 10 - TEMP$GAL_TAN_and_L_R_3
TEMP$GAL_TAN_and_L_R_4 <- 10 - TEMP$GAL_TAN_and_L_R_4
TEMP$GAL_TAN_and_L_R_7 <- 10 - TEMP$GAL_TAN_and_L_R_7

TEMP$GAL_TAN <- NA
for (i in 1:nrow(TEMP)) {
  
  count <-  7 - sum(is.na(TEMP[i,8:14]))
  
  TEMP[i,"GAL_TAN"] <- sum(TEMP[i,8:14]) / count
  
}


ggplot(TEMP, aes(GAL_TAN)) +
  geom_histogram(aes(y = ..density..), colour = 1, fill = "white") +
  geom_density(lwd = 1, colour = 4, fill = 4, alpha = 0.25) +
  xlim(0, 10) +
  theme_bw()
  ...
  title <- "GAL-TAN v. FBI"
  me <- mean(TEMP$GAL_TAN, na.rm = T)
  cor <- cor.test(TEMP$GAL_TAN, TEMP$FBI, method = "pearson")
  foot <- paste("Pearsons correlation = ", round(cor$estimate, 2), "; p-value = ", round(cor$p.value, 4), sep = "")
  
  
  
ggplot(TEMP, aes(FBI, GAL_TAN)) +
    geom_point(aes(colour = TEMP$type)) +
    geom_smooth(method=lm) +
    geom_hline(yintercept= me, linetype="dashed", alpha = 0.6) +
    ggtitle(title) +
...
As one might expect from the individual plots shown above, the overall trend seems to be that those who are more likely to encounter agreement are also more likely to be the most extreme in the GAL dimension. The green dots are pretty closely clustered together below the average GAL-TAN rating and we have only two observations above it.

Our red dots, however, vary a lot more, although the overall trend seems to be that they are more close to the TAN dimension. However, the most extreme TAN scores are not the ones that are also extreme in the negative end of the FBI.

Our "balanced" blue dots are also pretty widely distributed on the GAL-TAN axis, which would be expected.

---

What about the LEFT-RIGHT dimension?

Here, I have used the last 6 questions where the response scales are reversed so that 0 always indicates LEFT and 10 indicates right:

8. [Left-Right, REVERSED] Ympäristöystävällisempi Suomi, vaikka se tarkoittaisi alhaista talouskasvua tai ei kasvua ollenkaan
9. [Left-Right] Euroopan unioniin vähemmän sitoutunut Suomi
10. [Left-Right] Suomi, jossa on enemmän yksityisyrittäjyyttä ja markkinataloutta
11. [Left-Right] Suomi, jossa on alhaisempi verotus
12. [Left-Right, REVERSED] Suomi, jossa on pienemmät tuloerot
13. [Left-Right] Suomi, jossa on pienempi julkinen sektori

```{r echo=FALSE, message=FALSE, warning=FALSE}

TEMP$GAL_TAN_and_L_R_8 <- 10 - TEMP$GAL_TAN_and_L_R_8
TEMP$GAL_TAN_and_L_R_12 <- 10 - TEMP$GAL_TAN_and_L_R_12

TEMP$LEFT_RIGHT <- NA
for (i in 1:nrow(TEMP)) {
  
  count <-  6 - sum(is.na(TEMP[i,15:20]))
  
  TEMP[i,"LEFT_RIGHT"] <- sum(TEMP[i,15:20]) / count
  
}


ggplot(TEMP, aes(LEFT_RIGHT)) +
  geom_histogram(aes(y = ..density..), colour = 1, fill = "white") +
  geom_density(lwd = 1, colour = 4, fill = 4, alpha = 0.25) +
  xlim(0, 10) +
  theme_bw()
  ...
  title <- "LEFT-RIGHT v. FBI"
  me <- mean(TEMP$LEFT_RIGHT, na.rm = T)
  cor <- cor.test(TEMP$LEFT_RIGHT, TEMP$FBI, method = "pearson")
  foot <- paste("Pearsons correlation = ", round(cor$estimate, 2), "; p-value = ", round(cor$p.value, 4), sep = "")
  
  
  
ggplot(TEMP, aes(FBI, LEFT_RIGHT)) +
    geom_point(aes(colour = TEMP$type)) +
    geom_smooth(method=lm) +
    geom_hline(yintercept= me, linetype="dashed", alpha = 0.6) +
    ggtitle(title) +
...
The overall pattern seems to be pretty similar here, although the negative correlation is stronger and the p-value a lot smaller. 

Also, this time the majority of our "balanced" blues are below the average LEFT-RIGTH score, which would suggest that they are more likely to be leftists. Then again, they ALSO have the people who lean most extremely to the right, but it is difficult to know to what degree this is a small sample issue. 

#### Impact of FBI to self-reported GAL-TAN and LEFT-RIGHT

Lastly, lets also check it the patterns are similar when we look at the self-reported GAL-TAN and LEFT-RIGHT dimensions:

>Politiikassa puhutaan joskus arvoliberaaliudesta ja arvokonservatiivisuudesta. Käytämme asteikkoa, jossa nolla tarkoittaa eniten arvoliberaali ja kymmenen eniten arvokonservatiivinen. **Mihin kohtaa sijoittaisit itsesi tällä asteikolla?**

```{r echo=FALSE, message=FALSE, warning=FALSE}

DF_W1 <- read.csv("C:/Users/aleks/Documents/WORK/Bubble survey eOpinion Panel/FB 250 Pilot Data/Wave 1 pilot (Cleaned).csv", sep=";", comment.char="#")

DF_W1 <- DF_W1[,c("ResponseId", "POL_LIB_TRAD_1", "POL_v_o_1")]
...
ggplot(TEMP, aes(x=POL_LIB_TRAD_1))+
  geom_bar() +
  geom_text(stat='count', aes(label=..count..), vjust=-0.5, size = 3) +
  xlab("Self-reporter GAL (0) - TAN (10)") +
  ylab("Count") +
...
title <- "Self-reported GAL-TAN v. FBI"
me <- mean(TEMP$POL_LIB_TRAD_1, na.rm = T)
cor <- cor.test(TEMP$POL_LIB_TRAD_1, TEMP$FBI, method = "pearson")
foot <- paste("Pearsons correlation = ", round(cor$estimate, 2), "; p-value = ", round(cor$p.value, 4), sep = "")
  
  
  
ggplot(TEMP, aes(FBI, POL_LIB_TRAD_1)) +
    geom_point(aes(colour = TEMP$type)) +
    geom_smooth(method=lm) +
    geom_hline(yintercept= me, linetype="dashed", alpha = 0.6) +
    ggtitle(title) +
...
Looks pretty similar, but the correlation is a lot smaller and not significant.

What about LEFT-RIGHT:

>Politiikassa puhutaan joskus vasemmistosta ja oikeistosta. Käytämme asteikkoa, jossa nolla tarkoittaa eniten vasemmalla ja kymmenen eniten oikealla. **Mihin kohtaa sijoittaisit itsesi tällä asteikolla?**

```{r echo=FALSE, message=FALSE, warning=FALSE}

ggplot(TEMP, aes(x=POL_v_o_1))+
  geom_bar() +
  geom_text(stat='count', aes(label=..count..), vjust=-0.5, size = 3) +
  xlab("Self-reporter LEFT (0) - RIGHT (10)") +
  ylab("Count") +
  scale_x_continuous(limits=c(-1, 10), breaks=c(0:10)) +
  theme_bw()
...
title <- "Self-reported LEFT-RIGHT v. FBI"
me <- mean(TEMP$POL_v_o_1, na.rm = T)
cor <- cor.test(TEMP$POL_LIB_TRAD_1, TEMP$FBI, method = "pearson")
foot <- paste("Pearsons correlation = ", round(cor$estimate, 2), "; p-value = ", round(cor$p.value, 4), sep = "")
  
  
  
ggplot(TEMP, aes(FBI, POL_v_o_1)) +
    geom_point(aes(colour = TEMP$type)) +
    geom_smooth(method=lm) +
    geom_hline(yintercept= me, linetype="dashed", alpha = 0.6) +
    ggtitle(title) +
...
Ok, the correlation is yet again smaller and the p-value is a lot bigger too. The distribution starts to be more random in this case.

### Alternative

Next, I will do the whole thing again, but this time I will use FBI constructed from the alternative survey.

Since it now becomes more ambiguous to determine who our "balanced" people are, I will just use a simple cutoff point to determine them here:

* "Balanced" are those whose FBI ranges from [-1, 1]
...
lables <- c("ID", "Filter Bubble Index", "FBI type", "Monikulttuurinen Suomi, jossa suhtaudutaan suvaitsevaisesti muista maista tuleviin ihmisiin", "Suomi, jossa kristilliset arvot ovat suuremmassa roolissa", "Suomi, jossa miehet ja naiset ovat keskenään tasa-arvoisempia", "Suomi, jossa vahvistetaan seksuaalisten vähemmistöjen oikeuksia", "Suomi, jossa on enemmän lakia ja järjestystä", "Suomi, jossa vahvistetaan perinteisen ydinperheen asemaa", "Suomi, jossa maahanmuutto on laajamittaisempaa", "Ympäristöystävällisempi Suomi, vaikka se tarkoittaisi alhaista talouskasvua tai ei kasvua ollenkaan", "Euroopan unioniin vähemmän sitoutunut Suomi", "Suomi, jossa on enemmän yksityisyrittäjyyttä ja markkinataloutta", "Suomi, jossa on alhaisempi verotus", "Suomi, jossa on pienemmät tuloerot", "Suomi, jossa on pienempi julkinen sektori")
...
for (i in 8:ncol(TEMP)) {
  ...
  cor <- cor.test(TEMP[[i]], TEMP$FBI, method = "pearson")
  foot <- paste("Pearsons correlation = ", round(cor$estimate, 2), "; p-value = ", round(cor$p.value, 4), sep = "")
  p_list_A <- append(p_list_A, cor$p.value)
...
The overall pattern seems to be similar than what we had in the DEFAULT survey: TAN and LEFT are more more likely to be our green dots, and the green dots are also more closely clustered together.

However, it also seems that there are more elements of randomness in the alternative survey, or at least that the relationship between FBI and the policy questions are not as linear here:

- Average p-value in the DEFAULT survey: **0.187**, SD: **0.177**
  * 2 with p < 0.05
- Average p-value in the DEFAULT survey: **0.329**, SD: **0.391**
  * 3 with p < 0.05

But there are also some correlations that are much clearer in the alternative srurvey. In particular these two:

- [GAL-TAN] Suomi, jossa maahanmuutto on laajamittaisempaa
- [Left-Right] Euroopan unioniin vähemmän sitoutunut Suomi

Those two had the strongest correlations thus far.

#### Impact of FBI to GAL-TAN and LEFT-RIGHT

How do things look when we combine GAL-TAN and LEFT-RIGHT -dimensions together:
...
for (i in 1:nrow(TEMP)) {
  ...
  count <-  7 - sum(is.na(TEMP[i,4:10]))
  
  TEMP[i,"GAL_TAN"] <- sum(TEMP[i,4:10]) / count
...
Things seem pretty similar to one other:

- The overall correlation size and direction is the same
- The distribution of the red, green and blue dots also mimic each other

Or at least I don't notice any significant differences.

---

Next up, the LEFT-RIGHT dimension:
...
for (i in 1:nrow(TEMP)) {
  ...
  count <-  6 - sum(is.na(TEMP[i,11:16]))
  
  TEMP[i,"LEFT_RIGHT"] <- sum(TEMP[i,11:16]) / count
...
The pattern is again similar, but this time the DEFAULT survey has the stronger correlation.

#### Impact of FBI to self-reported GAL-TAN and LEFT-RIGHT

Lets just quickly see the self-reported versions:
...
Hmm, something unusual is going on, since for the first time we see a positive correlation on the self-reported LEFT-RIGHT dimension, which would suggest that those who are more likely to encounter agreement are actually those who also lean to the right. 

But the regression line is pretty close to the overall mean and the confidence interavall also overlaps it, so this might be just becose of our small sample size and with a bigger sample we would see a negative correlation.


### The followup media use question on the default survey 

The last thing I wanted to have in this update is to respond to this suggestion from Arto:

>Seeing what the followup questions on media use and encounters reveal would be good to know before we make a final decision, I think.

So, here are the followup media questions. I have also put the corresponding questions form the Alternative survey next to it, so you don't have to scroll up to find it.

```{r echo=FALSE, fig.height=10, fig.width=20}


TEMP1 <- DF1[,c("II_Q1_EXTENDED_1", "II_Q1_EXTENDED_2", "II_Q1_EXTENDED_3", "II_Q1_EXTENDED_4", "II_Q1_EXTENDED_5", "II_Q1_EXTENDED_6", "II_Q1_EXTENDED_7", "II_Q1_EXTENDED_8", "II_Q1_EXTENDED_9")]

for (i in 1:ncol(TEMP1)) {
  
  TEMP1[i][TEMP1[i] == ""] <- NA
  TEMP1[i][TEMP1[i] == "I do not use this media at all "] <- NA
  TEMP1[,i] <- factor(TEMP1[,i], levels = c("Extremely uncommon", "Fairly uncommon", "Neither common nor uncommon", "Fairly common", "Extremely common"))
  
  names(TEMP1)[i] <- as.character(var_label(DF1[names(TEMP1[i])]))
}

TEMP1$Type <- "Disagreement"

TEMP2 <- DF1[,c("ORIE_Q1_EXTENDED_1", "ORIE_Q1_EXTENDED_2", "ORIE_Q1_EXTENDED_3", "ORIE_Q1_EXTENDED_4", "ORIE_Q1_EXTENDED_5", "ORIE_Q1_EXTENDED_6", "ORIE_Q1_EXTENDED_7", "ORIE_Q1_EXTENDED_8", "ORIE_Q1_EXTENDED_9")]
for (i in 1:ncol(TEMP2)) {
  
  TEMP2[i][TEMP2[i] == ""] <- NA
  TEMP2[i][TEMP2[i] == "I do not use this media at all "] <- NA
  TEMP2[,i] <- factor(TEMP2[,i], levels = c("Extremely uncommon", "Fairly uncommon", "Neither common nor uncommon", "Fairly common", "Extremely common"))
  
  names(TEMP2)[i] <- as.character(var_label(DF1[names(TEMP2[i])]))
...
for (i in 1:9) {
  names(TEMP)[i] <- paste(names(TEMP)[i], " [No use = ", round(sum(is.na(TEMP[i]))/140*100, 2), " %]", sep = "")
}



likert_Qs <- likert(TEMP[1:9], grouping=TEMP$Type)
Plot_default <-plot(likert_Qs, ordered = F) + ggtitle("DEFAULT SURVEY (n = 70)")



grid.arrange(Plot_default, Plot_alternative + ggtitle("ALTERNATIVE SURVEY (n = 83)"), nrow = 1)

```

Comparing the two, it looks like the DEFAULT survey results in pretty much to the same results as our ALTERNATIVE survey.

The only big difference is in YOUTUBE, which curiously is more "Balanced" in the DEFAULT survey. In the ALTERNATIVE version, you can see that it is more common for people to encounter disagreement on Youtube than agreement, but this difference is not as clear in the DEFAULT survey.

But still, I think we will get pretty much the same results here no matter which survey version we will use.

## Second conclusions:

I think that all of these updates further confirm that the DEFAULT survey is the way to go. There are no differences between the overall patterns that we can observe between the two surveys, but it seems that the default survey is easier for people to answer and it is also easier to work with the data on the default survey. 
